...
def increase(book_id, base_root):
    ...
    query = "UPDATE Books SET quantity=quantity+1 WHERE id=" + book_id
    ...
    message = "The does not belong to this Library"

    # We try to return the book whose id is given
    try:
        cur.execute(query)
        cnx.commit()
...
def return_book():
    ...
    submit = Button(
        ...
        font=('Calibri', 15),
        relief=FLAT,
        command=lambda: increase(idField.get(), root)
    )
